...
class Message(models.Model):
    ...
    input = models.TextField(max_length=20000)
    ...
    create_time = models.DateTimeField(default=now)
    def __str__(self):
        return f"{self.friend.character.name} - {self.friend.me.user.username} -{self.user_message[:30]} -{localtime(self.create_time).strftime('%Y-%m-%d %H:%M:%S')}"